...
def load_dataset(config):
    if config.dataset == 'abilene':
        data = np.load('./datasets/abilene.npy')[:config.num_slots]
    ...
    if config.dataset == 'geant':
        data = np.load('./datasets/geant.npy')[:config.num_slots]
        thsh = np.percentile(data, config.quantile)
        data[data > thsh] = thsh
        data /= thsh
        return data.astype(np.float32)
    ...
    if config.dataset == 'harvard226':
        data = np.load('./datasets/harvard226.npy')[:config.num_slots]
        thsh = np.percentile(data, config.quantile)
        data[data > thsh] = thsh
        data /= thsh
        return data.astype(np.float32)
    ...
    if config.dataset == 'wsdream':
        data = np.load('./datasets/wsdream.npy')[:config.num_slots]
        thsh = np.percentile(data, config.quantile)
        data[data > thsh] = thsh
        data /= thsh
        return data.astype(np.float32)
